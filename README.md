...
本项目在限时竞赛环境中完成开发，经历了从数据清洗到环境攻坚的完整闭环，最终在服务器资源极度受限的情况下取得稳定成绩：

| 阶段 | 核心挑战 | 解决方案与成果 |
| :--- | :--- | :--- |
| **Day 1** | 数据噪声干扰 + GPU 资源排队 | 发现部分图片底部含媒体平台水印标签，为防止模型将其误学为天气特征，**统一裁剪所有图片底部区域**。当日因算力紧张无法启动训练。 |
| **Day 2** | 服务器过载 + 环境依赖冲突 | 平台卡顿至文件上传与终端均不可用；下午服务器升级后恢复，但官方运行环境缺失关键依赖，导致推理接口无法正常跑分。 |
| **Day 3** | 环境隔离 + 性能验证 | **将完整 Python 环境本地打包上传**，程序内引用自带环境绕过系统限制，成功打通推理链路。最终跑分**稳定在 90% 左右**。 |

> 💡 **经验沉淀**：在不可控的竞赛平台中，"环境自包含"比"代码最优"更重要。数据预处理阶段的噪声排查（如水印裁剪）对模型泛化能力的贡献，往往不亚于模型结构本身的改进。
...
-   **RGB 图片**: `1cut/{class_name}/{image_id}.jpg` （已裁剪底部水印区域）
...
5.  **环境自包含**: 若在受限平台部署，建议参照比赛经验，将完整 Python 环境打包上传，避免依赖冲突。
